...
define([
    ...
    return Backbone.View.extend({
        ...
        play: function() {
            if (Dragon.Collections.Tracks.length <= 0)
                return;
        ...
        follow: function() {
            Dragon.Display.playbackCursorFollowing = !Dragon.Display.playbackCursorFollowing;
            $('#follow').toggleClass('following');
        },

        handleKey: function(e) {
            if (!Dragon.Views.Menu.hotkeysEnabled)
               return;

            switch(e.which) {
                case 32:
                    if (e.type === 'keydown' && !this.pressingKey) {
                        if (Dragon.Player.playing)
                            $('#stop').trigger('click');
                        else
                            $('#play').trigger('click');
