    ...
    return (function() {
        ...
        Player.prototype.play = function() {
            ...
            if (Dragon.Views.Editor.isActiveTrack())
                this.playbackFrom -= Dragon.Views.Editor.getCursor();
            ...
            Dragon.Collections.Tracks.each(function(track) {
                var cid = track.cid,
                ...
                track.clips.each(function(clip) {
                    ...
                    if (Dragon.Views.Editor.isActiveTrack()) {
                        cursor = Dragon.Views.Editor.getCursor();
            ...
            this.playing = true;
            this.playbackPositionInterval = setInterval("Dragon.Player.updatePlaybackPosition(Dragon.Player.playbackFrom)", 50);
        };
        ...
        Player.prototype.stop = function() {
            if (this.playing) {
                ...
                Dragon.Views.Editor.unsetActiveTrack();
                Dragon.Views.PlaybackControls.updateTime();
                Dragon.Display.showPlaybackPosition(0);
            }

            if (typeof this.playbackPositionInterval !== 'undefined')
                clearInterval(this.playbackPositionInterval);
            Dragon.Display.hidePlaybackPosition();
        };
        ...
        Player.prototype.updatePlaybackPosition = function(startTime) {
            if (this.playing && typeof startTime !== 'undefined') {
                ...
                if (newTime >= Dragon.Collections.Tracks.first().get('length'))
                    $('#stop').trigger('click');
                else
                    Dragon.Display.showPlaybackPosition(Dragon.Display.sec2px(newTime));
            } else {
                Dragon.Display.hidePlaybackPosition();
            }
        }
            ...
            reader.onloadend = function(e) {
                if (e.target.readyState == FileReader.DONE) { // DONE == 2
                    ...
                    var onsuccess = function(audioBuffer) {
                        $(el).trigger('Dragon:fileLoaded', [audioBuffer, file]);
                    },
                    onerror = function() {
