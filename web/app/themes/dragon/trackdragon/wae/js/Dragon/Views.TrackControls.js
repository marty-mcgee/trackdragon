...
define([
    ...
    return Backbone.View.extend({
        ...
        volumeChange: function() {
            ...
            if (!Dragon.Collections.Tracks.isAnySolo() || this.model.get('solo'))
                Dragon.Player.volumeChange(volume, cid);  
        ...
        mute: function() {
            ...
            if (muted) {
                ...
                if (!Dragon.Collections.Tracks.isAnySolo() || this.model.get('solo'))
                    Dragon.Player.volumeChange(this.model.get('gain'), this.model.cid);
            } else {
                // track is beeing muted (only if is not set as solo track)
                if (!this.model.get('solo'))
                    Dragon.Player.volumeChange(0, this.model.cid);
            }
        },

        solo: function() {
            this.model.set('solo', !this.model.get('solo'));
            var soloTracks = Dragon.Collections.Tracks.filter(
                                function(model) { 
                                    return model.get('solo') === true;
                                }
                            ),
                otherTracks = Dragon.Collections.Tracks.filter(
                                function(model) {
            ...
            if (soloTracks.length === 0) {
                Dragon.Collections.Tracks.each(function(model) {
                    if (model.get('muted') === false) {
                        Dragon.Player.volumeChange(model.get('gain'), model.cid);
                    } else {
                        Dragon.Player.volumeChange(0, model.cid);
                    }
                });
            } else {
