...
define([
    ...
    'underscore',
    'backbone',
    'Dragon/Views.ClipDisplay',
    'Dragon/Views.EditableName',
    'drag_resize'
    ...
    return Backbone.View.extend({
        ...
        initialize: function() {
            ...
            this.model.collection.bind('Dragon:zoomChange', this.render);
            this.model.collection.bind('Dragon:scroll', this.scrollChange);
        ...
        render: function() {
            var left  = Dragon.Display.sec2px(this.model.get('trackPos')),
            ...
            $(this.el).children().detach().end()  // detach() deletes element's content but saves event listeners etc.
                ...
                .resizable({
                    ...
                    resize: function(e, ui) {
                        ...
                        Dragon.Views.Editor.movingOn();  // blocks the selection while resizing
                        ...
                        if (ui.originalPosition.left === ui.position.left) { // from right NOTE: should be ok now
                            ...
                            newEndTime = (Dragon.Display.px2sec(ui.size.width) + that.model.get('startTime')) % duration;
                            loop       = loop + Math.floor((that.model.get('endTime') + Dragon.Display.px2sec(ui.size.width) - end) / duration);
                        } else {  // from left
                            newStartTime = Dragon.Display.px2sec(ui.position.left) - that.model.get('trackPos');
                            ...
                            if (that.model.get('trackPos') <= 0.05) {   // clip is at the very beginning of the track
                                if (newStartTime > 0) { // resize --> direction to the right NOTE: should be ok now
                                    ...
                                    newTrackPos  = Dragon.Display.px2sec(ui.position.left);
                    ...
                    stop: function() {
                        ...
                        Dragon.Views.Editor.movingOff();
                        Dragon.Collections.Tracks.deleteSelection(from, to, trackCid, that.model.cid);
        ...
        startMoving: function() {
            Dragon.Views.Editor.movingOn();
        },

        updatePosition: function(e) {
            var from     = Dragon.Display.px2sec(e.target.offsetLeft),
            ...
            Dragon.Collections.Tracks.deleteSelection(from, to, trackCid, this.model.cid);
        ...
        scrollChange: function(e, ui) {
            var scrollLeft = Dragon.Views.Editor.scrollLeftOffset(),
                left       = Dragon.Display.px2sec(scrollLeft),
                trackPos   = (typeof ui !== 'undefined') ? Dragon.Display.px2sec(ui.position.left) : this.model.get('trackPos'),
                width      = this._clipWidth(),
                offset     = left - trackPos;

            if (left > trackPos && left < (trackPos + width)) {
                $(this.editableName.el).css('padding-left', Dragon.Display.sec2px(offset))
        ...
        _clipWidth: function() {
            return Dragon.Display.sec2px(this.model.clipLength());
        },
    });
});
